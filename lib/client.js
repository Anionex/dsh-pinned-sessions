...
function PinnedSessionRow({ actions, cancelRemoval, current, index, open, prepareRemoval, session, t, title, unpin, workspace }) {
  ...
  (0, import_react3.useLayoutEffect)(() => {
    ...
    let observer;
    let disposed = false;
    const items = () => menu === void 0 ? [] : [...menu.querySelectorAll('[role="menuitem"]:not(:disabled)')];
    ...
    const install = () => {
      if (disposed) return;
      menu = [...document.querySelectorAll('[role="menu"]')].find((candidate) => !menusBeforeOpen.current.has(candidate));
